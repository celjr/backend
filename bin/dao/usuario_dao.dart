...
class UsuarioDAO implements DAO<UsuarioModel> {
  ...
  Future<bool> delete(int id) async {
    var result = await _dbConfiguration.execQuery('DELET FROM  usuarios WHERE id = ?', [id]);
    return result.affectedRows > 0;
  }
  ...
  Future<bool> update(UsuarioModel value) async {
    var result = await _dbConfiguration.execQuery(
        'update usuarios set nome = ? ,set  password = ? where id = ?',
  ...
  Future<UsuarioModel?> findByEmail(String email) async {
    var result =
        await _dbConfiguration.execQuery('SELECT * FROM usuarios WHERE email= ?', [email]);
    return result.affectedRows == 0 ? null : UsuarioModel.fromEmail(result.first.fields);
  }
